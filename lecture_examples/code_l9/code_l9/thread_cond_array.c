...
int main(void) {
  ...
  assert((pthread_cond_init(&thread_array->cond,NULL)) == 0);
  ...
  for(i=MAXTHREADS-1;i > -1;i--) {
    ...
    fflush(stdout);
    printf("--- Mama kicking all threads ---\n");
    assert((pthread_cond_broadcast(&thread_array->cond)) == 0);
    assert((pthread_mutex_unlock(&thread_array->mutex)) == 0);
  ...
  fflush(stdout);
  assert((pthread_cond_destroy(&thread_array->cond)) == 0);
